	...
	["NikkoClient_var_GroupHexegon", "onEachFrame",{
		{
			...
			//(group _x) == (group player) AND (alive _x) AND
			if ((_distance < 2500)) then {
				private _pos = (_unit modelToWorldVisual (_unit selectionPosition "pelvis"));
				_pos set [2, (_pos select 2) + 0.2];
				drawIcon3D [format ["%1\textures\icons\GroupHexegon.paa",str missionConfigFile select [0, count str missionConfigFile - 16]],[1,1,1,1],_pos,1,1,0,format ['%1 | %2m', name _unit, (_metres - (_metres % 1))],2,0.03,"PuristaBold"];
			};
